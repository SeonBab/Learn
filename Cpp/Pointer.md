...
위의 void* p는 자료형이 규정되어 있지 않는 것으로 누구나 가리킬 수 있다는 특징이 있지만 타입이 없어서 역참조가 불가능하며 연산도 안됩니다.
...
선언만 되고 초기화 되지 않았다면 포인터는 쓰레기값을 가지게 됩니다. 이 상태에서 메모리의 내용을 변경한다면 문제가 생길 수 있으며 초기화를 사용하거나 nullptr을 사용해야합니다.

포인터 값에 `0`값을 저장하면 아무것도 지정하지 않고 있다는 의미가 됩니다. 하지만 `0`은 숫자 리터럴이며 `int`자료형 숫자값이라 포인터의 자료형과는 의미가 다르므로 C++11 표준 이후 `nullptr` 사용을 권합니다. \
...
배열의 이름을 포인터처럼 사용할 수 있으며 포인터를 배열의 이름처럼 사용할 수 있습니다. 즉, 배열의 이름이 주소로 해석되며, 배열의 첫 번째 요소의 주소와 같습니다.
...
std::cout << arr[0] << arr[1] << arr[2] << std::endl;   // 배열의 이름을 이용
std::cout << ptr_arr[0] << ptr_arr[1] << ptr_arr[2] << std::endl;   // 포인터를 이용

std::cout << sizeof(arr) << std::endl;  // 배열의 이름을 이용
std::cout << sizeof(ptr_arr);   // 포인터를 이용
```

각 인덱스에 접근 할 때 포인터와 배열 둘 다 같은 출력이 됩니다. 하지만 배열의 크기를 계산할 경우 차이가 발생합니다. 배열의 이름을 사용해 크기를 계산할 경우 배열의 크기가 int형 배열 요소 3개 크기인 12바이트가 출력됩니다. 포인터를 이용한 크기 계산에서는 배열의 크기가 아닌 포인터 변수의 크기가 출력되는 차이가 생깁니다.
...
`const`라는 키워드는 변수를 상수화 할 때 사용하는 키워드입니다. 이 키워드를 포인터에도 사용이 가능한데, `const`의 위치에 따라 의미가 달라집니다.

가장 먼저 변수가 `const`일 경우입니다.

포인터를 지정하려고 하면 Compile Error가 발생합니다. 이유는 `*ptr = 7`이런 식으로 값을 변경할 수 있어 상수라는 개념에 위반돼 이런 포인터는 허용되지 않습니다.

```
const int number = 10;
int* ptr = &number; // Compile Error 발생
```

포인터의 자료형 앞에 `const` 키워드가 사용됐을 경우입니다. \
변수를 상수로 가리키는 포인터로 역참조를 이용해 해당 값을 변경할 수 없게 됩니다. \
다른 주소를 저장 가능합니다.
...
즉 `ptr`이 상수가 되며, 가리키는 값은 상수가 되어 가리키는 주소를 역참조해 `number`의 값을 변경할 수 없습니다.


포인터의 자료형 뒤에 `const` 키워드가 사용됐을 경우입니다. \
포인터 자체를 상수로 만들며 상수 포인터는 초기화 후 가리키는 주소를 변경할 수 없는 포인터 입니다. \
...
구조체 역시 포인터를 사용할 수 있으며 예제는 다음과 같습니다.
...
int main()
{
    ...
    Status* pStatus = &myStatus;    // Status형 포인터 선언

    std::cout << (*pStatus).HP << std::endl;
}
```

`Status`형 포인터를 선언과 동시에 `myStatus` 변수의 주소로 초기화했습니다. \
그 다음 구조체의 포인터가 가르키는 값에 접근 할 때 `*pStatus.HP`로 접근을 하게 되면 에러가 나며,  우선순위가 가장 높은 `.`연산자가 우선적으로 실행되며, 두번째인 `*`연산자가 실행됩니다. 따라서 `*(pStatus.HP)`로 잘못된 표현이 되므로 `(*pStatus).HP`를 사용을 해야합니다. \
이러한 구조체 포인터 표현의 모호함을 없애기 위해 `->`라는 구조체 포인터 연산자가 정의되어 있습니다.
