...
예제처럼 포인터를 지정하려고 하면 Compile Error가 발생합니다. 이유는 `*ptr = 7`이런 식으로 값을 변경할 수 있어 상수라는 개념에 위반돼 이런 포인터는 허용되지 않습니다.

포인터의 자료형 앞에 `const`가 붙어있을 경우입니다.

```
const int* ptr = &number;
```

상수에 대한 포인터로 `int *ptr`을 상수화 하겠다는 뜻입니다. 즉 `*ptr`이 상수가 되며, 가리키는 값이 상수가 되어 가리키는 `number`의 값을 변경 할 수 없게 됩니다.
