...
override 지시어는 컴파일러가 오버라이딩을 확인하도록 지시해 해당 함수가 상속 받아 오버라이딩이 가능한지, 그 함수의 이름과 매개 변수등이 맞는지 컴파일러가 확인하라고 할 수 있다.

키워드를 붙일 수 있는 조건은 다음과 같다.
+ 기본 클래스 함수가 반드시 가상 함수 이어야한다.
+ 기본 함수와 파생 함수의 함수 명이 동일해야한다. (소멸자는 제외한다.)
+ 기본 함수와 파생 함수의 한정자가 동일해야한다.
...
위와 같은 경우 `func2()`함수를 컴파일러가 오버라이딩을 확인해 컴파일 오류가 발생할 것이다.

final 지시어는 클래스의 상속을 막고, 가상 함수의 재정의를 방지하는 키워드로 가상 함수에서는 더이상 재정의 할 수 없도록 지정한다.

final 지시어가 붙은 클래스와 가상 함수는 상속 받아 사용하려 할 때 컴파일 오류가 발생한다.
...
위와 같은 경우 `func1`함수를 재정의하려 해 오류가 발생 할 것이다.